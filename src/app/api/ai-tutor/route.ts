...
const RAW_MODE_PROMPTS: Record<TutorMode, string> = {
...
// One shared formatting contract, appended to every mode above. The chat UI
// now renders real Markdown (bold/italic/lists/code all become actual
// elements — see src/components/shared/markdown.tsx), so these rules exist
// to keep the OUTPUT clean and sparing rather than to make it "look
// technically like Markdown": the earlier bug students hit wasn't really
// that the model used Markdown, it's that nothing rendered it — but a model
// left unconstrained will still over-bold and over-decorate once it knows
// formatting "works", so this stays even with the render fix in place.
const FORMAT_CONTRACT = `

Formatting rules — the app renders this as real Markdown, so they matter:
- **Bold** only genuinely key terms, the final answer, or a short header — a few bolded phrases per response at most, never whole sentences.
- *Italics* only for a single word of emphasis or a term on first use — use rarely.
- Bullet ("- ") or numbered ("1. ") lists for steps and enumerations, not for every sentence — plain prose is fine for a single point.
- \`code\` formatting only for actual formulas, equations, chemical symbols, or values with units — never for ordinary words.
- Headings (## / ###) only in long, multi-section answers (essay reviews, multi-step derivations) — skip them in short answers.
- No decorative asterisk/dash borders, no ALL CAPS for emphasis, no stray trailing asterisks.
- Short paragraphs (2-4 sentences); prefer a list over a dense paragraph once you're covering more than one point.`;

const MODE_PROMPTS: Record<TutorMode, string> = Object.fromEntries(
  (Object.entries(RAW_MODE_PROMPTS) as [TutorMode, string][]).map(([mode, prompt]) => [mode, prompt + FORMAT_CONTRACT])
...
export async function POST(req: NextRequest) {
  ...
  if (limit.limited) {
    const friendly =
      limit.reason === "burst"
        ? `You're sending messages faster than I can think! Give me about ${limit.retryAfterSeconds}s and try again.`
        : `You've hit today's AI Tutor limit. It resets in about ${Math.ceil(limit.retryAfterSeconds / 3600)}h — plenty of non-AI practice (flashcards, MCQs, mock tests) still works in the meantime.`;
    return NextResponse.json(
      { reply: friendly, error: true, rateLimited: true },
