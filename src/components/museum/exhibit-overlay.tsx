...
import { X, ChevronLeft, MousePointerClick, RotateCw, Frame, ArrowRight, Check, Layers } from "lucide-react";
import type { ExhibitDefinition } from "./exhibits/types";

// Carries the "next stop" behavior for a 3D exhibit opened mid-walkthrough —
// the same shape StationData already uses for text stations, so the Focus
// viewer's Continue Tour / Finish Tour button (below) has something to call.
export type ExhibitTour = { onContinueTour?: () => void; isLastStop?: boolean };
...
export function ExhibitOverlay({
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-background/95 backdrop-blur-md flex flex-col animate-float-up">
      {/* top bar */}
      <div className="flex items-center gap-3 px-4 sm:px-6 py-3 border-b border-border glass-clear">
        <button
          onClick={onClose}
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0">
        {/* Stage */}
        <div className="flex-1 relative min-h-[45vh] lg:min-h-0" style={{ background: "radial-gradient(circle at 50% 40%, #0c1014 0%, #030405 70%)" }}>
          {exhibit.kind === "3d" && exhibit.Model ? (
            ...
            >
              <ambientLight intensity={0.32} color="#dff2f7" />
              <directionalLight position={[4, 8, 4]} intensity={0.85} castShadow shadow-mapSize={[1024, 1024]} shadow-bias={-0.0004} />
              ...
              <Environment resolution={256} frames={1}>
                <Lightformer intensity={1.6} color="#eef8fb" position={[0, 6, 0]} rotation={[Math.PI / 2, 0, 0]} scale={[8, 8, 1]} />
                <Lightformer intensity={0.6} color={exhibit.accent} position={[-5, 3, 0]} rotation={[0, Math.PI / 2, 0]} scale={[6, 4, 1]} />
                <Lightformer intensity={0.5} color="#ffffff" position={[5, 3, 0]} rotation={[0, -Math.PI / 2, 0]} scale={[6, 4, 1]} />
              </Environment>
              <group position={[0, 0.2, 0]}>
                <exhibit.Model selectedPart={selectedPart} onSelectPart={(id) => setSelectedPart(id === selectedPart ? null : id)} explode={explode} />
              </group>
              ...
              <OrbitControls
                ...
                autoRotate={!selectedPart && explode === 0}
                autoRotateSpeed={0.6}
              />
          ...
          {/* Explode slider — only for exhibits that opt in (heart, neuron) */}
          {exhibit.kind === "3d" && exhibit.explodable && (
            <div className="glass-clear glass-interactive absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-3 rounded-full px-4 py-2 w-[min(90%,340px)]">
              <Layers className="size-3.5 shrink-0" style={{ color: exhibit.accent }} />
              <input
                type="range"
                min={0}
                max={100}
              ...
              />
              <span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground shrink-0 w-14 text-right">
                {explode === 0 ? "Assembled" : explode >= 0.99 ? "Exploded" : `${Math.round(explode * 100)}%`}
              </span>
            </div>
          )}

          {/* hint */}
          <div className="glass-clear absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-4 text-[11px] text-muted-foreground rounded-full px-4 py-2 pointer-events-none">
            {exhibit.kind === "3d" ? (
              <>
