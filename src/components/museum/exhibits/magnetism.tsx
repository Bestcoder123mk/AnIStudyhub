...
// Bar magnet placement — module-level since it's a fixed layout constant,
// not derived from props/state (also keeps the `fieldLines` useMemo below
// honestly dependency-free instead of silently relying on a fresh-but-
// identical array literal every render).
const MAGNET_POS: [number, number, number] = [-0.7, 0.4, 0];
const MAGNET_LEN = 1.6;
...
function MagnetismModel({ selectedPart, onSelectPart, preview }: ExhibitModelProps) {
    ...
    for (let i = 0; i < arcCount; i++) {
      ...
      for (let r = 0; r < radii.length; r++) {
        ...
        for (let j = 0; j <= seg; j++) {
          ...
          pts.push(new THREE.Vector3(x + MAGNET_POS[0], y + MAGNET_POS[1], z + MAGNET_POS[2]));
        }
  ...
  return (
    <group ref={group} scale={scale}>
      ...
      <group position={MAGNET_POS}>
        <mesh position={[MAGNET_LEN / 4, 0, 0]} castShadow onClick={click("bar-magnet")} onPointerOver={over("bar-magnet")} onPointerOut={out}>
          <boxGeometry args={[MAGNET_LEN / 2, 0.35, 0.35]} />
        ...
        {!preview && (
          <>
            <Html position={[MAGNET_LEN / 2 + 0.18, 0.16, 0]} center distanceFactor={8}>
