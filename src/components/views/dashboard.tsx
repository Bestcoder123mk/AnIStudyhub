...
import {
  computeChapterMastery, computeReadiness, recoverMyMarks, generateDailyPlan,
  estimatedMarksRecoverable, trackQuestionCountByChapter, examCountdownTier,
...
export function DashboardView() {
  ...
  // ===== Today's Mission (Command Center) =====
  // "How close am I to 95%? What's stopping me? What should I do right
  // now?" — the three questions the review doc argued should sit at the
  // center of the app, above the gamification layer.
  const chapterStatsScience = useStudyStore((s) => s.chapterStats.science);
  const diffStatsScience = useStudyStore((s) => s.diffStats.science);
  const recallStatsScience = useStudyStore((s) => s.recallStats.science);
  const mockHistory = useStudyStore((s) => s.mockHistory);
  const mistakes = useStudyStore((s) => s.mistakes);
  const openedChapters = useStudyStore((s) => s.openedChapters);
  const missionChapters = useMemo(() => getTrackChapters("science"), []);
  const missionQCounts = useMemo(() => trackQuestionCountByChapter("science"), []);
  const missionMastery = useMemo(
    () => computeChapterMastery(missionChapters, chapterStatsScience, missionQCounts, recallStatsScience),
    [missionChapters, chapterStatsScience, missionQCounts, recallStatsScience]
  );
  const missionReadiness = useMemo(
    () => computeReadiness({
      chapters: missionMastery,
      diffStats: diffStatsScience,
      recallStats: recallStatsScience,
      mockHistory: mockHistory.filter((m) => m.track === "science"),
      mistakes: mistakes.filter((m) => m.track === "science"),
      totalAttempted: totalAnswered,
      openedChapters: openedChapters.length,
      totalChapters: missionChapters.length,
    }),
    [missionMastery, diffStatsScience, recallStatsScience, mockHistory, mistakes, totalAnswered, openedChapters.length, missionChapters.length]
  );
  const missionRecover = useMemo(() => recoverMyMarks(missionMastery, 3), [missionMastery]);
  const sciMistakeCount = useMemo(() => mistakes.filter((m) => m.track === "science").length, [mistakes]);
  const missionTasks = useMemo(
    () => generateDailyPlan(missionRecover, sciMistakeCount, "mcq", "mistakes", "short", "mock"),
    [missionRecover, sciMistakeCount]
  );
  const missionMinutes = missionTasks.reduce((a, t) => a + t.minutes, 0);
  const missionMarks = useMemo(() => estimatedMarksRecoverable(missionTasks, missionRecover), [missionTasks, missionRecover]);
  ...
  const countdown = useMemo(() => examCountdownTier(examDays), [examDays]);
    ...
    <div className="space-y-6">
      ...
      <header className="relative overflow-hidden rounded-3xl border border-border/60 p-6 sm:p-8 animate-rise">
        ...
        <div className="relative z-10 space-y-2">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-foreground">Welcome back, {findTitle(equippedTitle)?.label || "Scholar"}! 🚀</h1>
      ...
      {/* Today's Mission — Command Center */}
      <Card className="glass rounded-2xl animate-rise">
        <CardHeader className="pb-2">
          <CardTitle className="text-lg flex items-center gap-2"><Compass className="size-4 text-foreground" /> Today&apos;s Mission</CardTitle>
        </CardHeader>
        <CardContent>
          {countdown.key !== "unset" && (
            <div className="mb-4 flex items-start gap-2 rounded-lg border border-border/60 p-2.5 text-xs">
              <span className="text-base leading-none">{countdown.icon}</span>
              <div>
                <span className="font-semibold">{countdown.headline}.</span>{" "}
                <span className="text-muted-foreground">{countdown.message}</span>
              </div>
            </div>
          )}
          <div className="grid md:grid-cols-[auto_1fr] gap-5 items-start">
            <div className="flex items-center gap-3 shrink-0">
              <div className="text-4xl font-bold tabular-nums">
                {missionReadiness.readinessPct === null ? "—" : `${missionReadiness.readinessPct}%`}
              </div>
              <div className="text-xs text-muted-foreground max-w-[9rem]">
                {missionReadiness.readinessPct === null
                  ? "Answer a few questions to unlock your readiness score."
                  : "Exam Readiness — see Analytics for the full breakdown."}
              </div>
            </div>
              ...
              {missionTasks.length > 0 ? (
                <>
                  <p className="text-xs text-muted-foreground">
                    {missionMarks > 0 ? `Roughly +${missionMarks} marks recoverable today · ` : ""}
                    {missionMinutes}-minute plan built from your weakest evidence:
                  </p>
                ...
                <p className="text-sm text-muted-foreground">No weak chapters detected yet — keep practicing and this will fill in with a targeted plan.</p>
              )}
            </div>
          </div>
        </CardContent>
        ...
        <Card className="card-premium grain lg:col-span-2 rounded-2xl animate-rise stagger-1">
          ...
          <CardContent className="space-y-5">
            ...
            <p className="text-xs text-muted-foreground pt-1">
              {MCQS.length} MCQs total across the Science track · keep grinding to complete each subject.
            </p>
          </CardContent>
        </Card>
                  ...
                  {goals.map((g) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Button variant="ghost" size="icon" className="size-8 text-muted-foreground hover:text-rose-400" onClick={() => removeGoal(g.id)} aria-label="Remove goal">
