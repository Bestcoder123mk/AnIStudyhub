...
import { SSC_FLASHCARDS, SSC_CHAPTERS } from "@/lib/study-data";
...
export function SscFlashcardsView() {
  ...
  const deckCards = useMemo(() => {
    const items =
      deck === "0"
        ? SSC_FLASHCARDS
        : SSC_FLASHCARDS.filter((c) => c.ch === parseInt(deck, 10));
    return items.map((c) => ({ card: c, origIdx: SSC_FLASHCARDS.indexOf(c) }));
  }, [deck]);

  const safeIdx = deckCards.length > 0 ? Math.min(idx, deckCards.length - 1) : 0;
  const current = deckCards[safeIdx];

  const subj: SscSubject = (current?.card.subj as SscSubject) || "hist";
  ...
  const rate = (quality: 2 | 3 | 5) => {
    if (!current) return;
    reviewFlash("ssc");
    rateSrCard(`sscflash-${current.card.ch}-${current.origIdx}`, quality);
    setTimeout(() => {
      setFlipped(false);
      setIdx((i) => (i + 1) % deckCards.length);
    }, 220);
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">SSC Flashcards ⚡</h1>
        <p className="text-sm text-muted-foreground mt-1">
          20 cards · Click to flip · Rate yourself · +5 XP per review
        </p>
      </header>
