...
import { ENG_MCQS, ENG_CHAPTERS, type EngBook } from "@/lib/english-data";
...
const ACCENT = SUBJECT_META.english.accent; // single source of truth — was a duplicated hex literal
const TEST_MINS = 20;
const QUESTION_COUNT = 10;
const LETTERS = ["A", "B", "C", "D"];

type Preset = "all" | EngBook;
...
const PRESETS: { key: Preset; label: string }[] = [
  ...
  { key: "First Flight", label: "🕊️ First Flight" },
...
function chapterTitle(ch: number) {
  return ENG_CHAPTERS.find((c) => c.id === ch)?.title ?? `Chapter ${ch}`;
}

function bookOfCh(ch: number): EngBook | undefined {
  return ENG_CHAPTERS.find((c) => c.id === ch)?.book;
}

export function EngMockView() {
  ...
  const availablePyqCount = useMemo(() => {
    const pool = preset === "all" ? ENG_MCQS : ENG_MCQS.filter((m) => bookOfCh(m.ch) === preset);
  ...
  const startTest = () => {
    let pool =
      preset === "all"
        ? ENG_MCQS
        : ENG_MCQS.filter((m) => bookOfCh(m.ch) === preset);
    if (pyqOnly) pool = pool.filter((m) => m.pyq === true);
    if (pool.length === 0) {
  ...
  const submitTest = () => {
    ...
    if (xpGain > 0) addXp(xpGain, "English mock test");
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {ENG_MCQS.length} MCQs available · Timed {fmtMins(TEST_MINS)} · {QUESTION_COUNT} questions · Auto-graded
          </p>
        </div>
            ...
            {PRESETS.map((p) => {
              ...
              return (
                <Button
                  ...
                  style={
                    active
                      ? { backgroundColor: ACCENT, color: "#1a0613", borderColor: ACCENT }
                      : undefined
                  }
                >
                  {p.label}
        ...
        <Card className="glass p-5 gap-0">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <Button onClick={startTest} size="lg" style={{ backgroundColor: ACCENT, color: "#1a0613" }}>
              <Play /> Start Test
            </Button>
          </div>
        </Card>
      ...
      <div className="space-y-4">
            ...
            {questions.map((qq) => {
              ...
              return (
                <button
                  ...
                  style={
                    isCurrent
                      ? { backgroundColor: ACCENT, borderColor: ACCENT, color: "#1a0613" }
                      : ans
        ...
        <div style={{ "--sc": ACCENT } as CSSProperties}>
          <Card className="glass p-5 gap-0">
            ...
            <p className="font-semibold text-lg leading-snug mb-4 whitespace-pre-wrap">{q.q}</p>
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-border">
              ...
              <Button
                ...
                style={{ backgroundColor: ACCENT, color: "#1a0613" }}
              >
                Next <ArrowRight />
              </Button>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="rounded-md bg-muted/40 border border-border p-3">
                    ...
                    <Markdown text={q.exp} compact className="text-xs" />
                  </div>
                </div>
              );
            })}
