...
import { useStudyStore, SUBJECT_META, getTrackXp, type Track } from "@/store/use-study-store";
...
/* ---------- helpers ---------- */
// See parent.tsx / use-study-store.ts's getTrackXp — this was a third,
// identical copy of the same resolver.
...
export function TeacherView() {
  ...
  // `store` used to be built once per render and passed into the memo as a
  // dependency — but a fresh object literal every render meant the memo
  // recomputed every render anyway (never actually cached anything). Build
  // the tiny xp-source object *inside* the memoized callback instead, and
  // depend on the actual primitives it's built from.
  const myTotalXp = useMemo(() => {
    const xpSource = { totalXp: _totalXp, ssc: { totalXp: _sscXp }, subjectStats: _subjectStats };
    return (["science", "ssc", "maths", "english", "sanskrit"] as Track[]).reduce((sum, t) => sum + xpForTrack(xpSource, t), 0);
  }, [_totalXp, _sscXp, _subjectStats]);
  const myAccuracy = _totalAnswered > 0
    ? Math.round((_totalCorrect / _totalAnswered) * 100)
    : 0;
