...
export function SocialView() {
  ...
  // Was `s.totalXp + s.ssc.totalXp` — silently dropped Maths/English/Sanskrit
  // XP, same bug fixed elsewhere via the canonical `getTrackXp` resolver
  // (see its comment in use-study-store.ts). A student focused on those
  // subjects would see a misleadingly low "Your Social Rank" XP here.
  const myScienceXp = useStudyStore((s) => s.totalXp);
  const mySscXp = useStudyStore((s) => s.ssc.totalXp);
  const mySubjectStats = useStudyStore((s) => s.subjectStats);
  const totalXp = ALL_TRACKS.reduce(
    (sum, t) => sum + getTrackXp({ totalXp: myScienceXp, ssc: { totalXp: mySscXp }, subjectStats: mySubjectStats }, t),
    0
  );
