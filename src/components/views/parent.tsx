...
import { useStudyStore, SUBJECT_META, getTrackXp, DIFF_TIERS, type Track, type DiffTier } from "@/store/use-study-store";
...
import {
  ...
  BarChart3, Lightbulb, Award, AlertCircle, BookOpen, Info, Layers,
} from "lucide-react";

const DIFF_COLORS: Record<DiffTier, string> = { easy: "#34d399", medium: "#fbbf24", hard: "#fb7185" };

/* ---------- helpers ---------- */
// XP-per-track resolution now lives in one place (the store's getTrackXp) —
// this used to be a locally hand-rolled copy, duplicated identically across
// this file, career.tsx, and teacher.tsx. Two other call sites in the app
// (the old xp-bar.tsx and footer.tsx) had a *different*, incomplete copy
// that forgot the Maths/English/Sanskrit case entirely and silently fell
// back to Science's XP — see getTrackXp's own comment in the store.
const xpForTrack = getTrackXp;
...
export function ParentView() {
  ...
  const _diffStats = useStudyStore((s) => s.diffStats);
  ...
  // Accuracy by difficulty, summed across all 5 subjects — the same tiers
  // that power the Adaptive quiz mode, one level zoomed out.
  const diffTotals = useMemo(() => {
    const totals = { easy: { correct: 0, attempted: 0 }, medium: { correct: 0, attempted: 0 }, hard: { correct: 0, attempted: 0 } };
    (Object.keys(_diffStats) as Track[]).forEach((t) => {
      DIFF_TIERS.forEach((tier) => {
        totals[tier].correct += _diffStats[t][tier].correct;
        totals[tier].attempted += _diffStats[t][tier].attempted;
      });
    });
    return totals;
  }, [_diffStats]);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="glass">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg flex items-center gap-2">
              <Layers className="size-4 text-cyan-400" /> Accuracy by difficulty
            </CardTitle>
            <CardDescription>Across all 5 subjects — powers the ⚡ Adaptive quiz mode</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {DIFF_TIERS.map((tier) => {
              const d = diffTotals[tier];
              const p = d.attempted > 0 ? Math.round((d.correct / d.attempted) * 100) : 0;
              return (
                <div key={tier}>
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="font-medium capitalize">{tier}</span>
                    <span className="tabular-nums text-muted-foreground">
                      {d.attempted > 0 ? `${p}% · ${d.correct}/${d.attempted}` : "No data yet"}
                    </span>
                  </div>
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div
                      className="h-full rounded-full transition-all duration-500"
                      style={{ width: `${p}%`, backgroundColor: DIFF_COLORS[tier] }}
                    />
                  </div>
                </div>
              );
