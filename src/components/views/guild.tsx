...
import { useStudyStore, ALL_TRACKS, getTrackXp } from "@/store/use-study-store";
...
export function GuildView() {
  ...
  // "You"'s xp inside `guild.members` is a one-time snapshot taken at
  // `joinGuild()` (see the store) — it never updates again, so the
  // leaderboard would show your join-day XP forever no matter how much
  // more you study. It was also only ever science + SSC, silently
  // ignoring Maths/English/Sanskrit XP (the same bug `getTrackXp` was
  // introduced elsewhere to fix). Compute the real, current, all-subject
  // total here and use it to override the frozen snapshot at display time.
  const myTotalXp_science = useStudyStore((s) => s.totalXp);
  const myTotalXp_ssc = useStudyStore((s) => s.ssc.totalXp);
  const myTotalXp_subjectStats = useStudyStore((s) => s.subjectStats);
  const myLiveTotalXp = ALL_TRACKS.reduce(
    (sum, t) => sum + getTrackXp({ totalXp: myTotalXp_science, ssc: { totalXp: myTotalXp_ssc }, subjectStats: myTotalXp_subjectStats }, t),
    0
  );
  ...
  const liveMembers = guild.members.map((m) => (m.name === "You" ? { ...m, xp: myLiveTotalXp } : m));
  const sortedMembers = [...liveMembers].sort((a, b) => b.xp - a.xp);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        <Card className="glass rounded-xl"><CardContent className="pt-3 pb-3 text-center"><div className="text-lg font-bold text-primary">{liveMembers.reduce((a, b) => a + b.xp, 0)}</div><div className="text-[9px] text-muted-foreground uppercase">Guild XP</div></CardContent></Card>
