...
// Build the deck: one card per chapter (front: title + "themes?", back: keypts joined).
function buildEngCards(): EngCard[] {
  return ENG_CHAPTERS.map((ch, idx) => ({
    ch: ch.id,
    chTitle: ch.title,
    author: ch.author,
    idx,
    front: `"${ch.title}" by ${ch.author} — themes?`,
    back: ch.keypts.join("\n\n"),
  }));
}

const ALL_CARDS = buildEngCards();

export function EngFlashcardsView() {
  ...
  const rate = (quality: 2 | 3 | 5) => {
    if (!current) return;
    reviewFlash("english");
    rateSrCard(`engflash-${current.ch}-${current.idx}`, quality);
    setTimeout(() => {
      setFlipped(false);
      setIdx((i) => (i + 1) % deckCards.length);
    }, 220);
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {ALL_CARDS.length} cards · one per chapter · themes, characters &amp; motifs · Click to flip · +5 XP per review
        </p>
      </header>
