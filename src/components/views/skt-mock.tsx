...
import { SKT_MCQS, SKT_CHAPTERS } from "@/lib/sanskrit-data";
...
const PRESETS: { key: Preset; label: string }[] = [
  ...
  { key: "first", label: "🕉️ Ch 1-6 (First Half)" },
  { key: "second", label: "🪔 Ch 7-12 (Second Half)" },
];
...
function chapterTitle(ch: number) {
  return SKT_CHAPTERS.find((c) => c.id === ch)?.title ?? `Chapter ${ch}`;
}

export function SktMockView() {
  ...
  const pyqFilteredPool = (base: typeof SKT_MCQS) => (pyqOnly ? base.filter((m) => m.pyq === true) : base);

  const availablePyqCount = useMemo(() => {
    const pool =
      preset === "all" ? SKT_MCQS : preset === "first" ? SKT_MCQS.filter((m) => m.ch >= 1 && m.ch <= 6) : SKT_MCQS.filter((m) => m.ch >= 7 && m.ch <= 12);
    return pool.filter((m) => m.pyq).length;
  }, [preset]);
  ...
  const startTest = () => {
    const basePool =
      preset === "all"
        ? SKT_MCQS
        : preset === "first"
        ? SKT_MCQS.filter((m) => m.ch >= 1 && m.ch <= 6)
        : SKT_MCQS.filter((m) => m.ch >= 7 && m.ch <= 12);
    const pool = pyqFilteredPool(basePool);
    if (pool.length === 0) {
  ...
  const submitTest = () => {
    ...
    if (xpGain > 0) addXp(xpGain, "Sanskrit mock test");
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {SKT_MCQS.length} MCQs available · Timed {fmtMins(TEST_MINS)} · {QUESTION_COUNT} questions · Auto-graded
          </p>
        </div>
            ...
            {PRESETS.map((p) => {
              ...
              return (
                <Button
                  ...
                  style={
                    active
                      ? { backgroundColor: ACCENT, color: "#1a1306", borderColor: ACCENT }
                      : undefined
                  }
                >
                  {p.label}
        ...
        <button
          ...
          className="w-full text-left card-premium tap-lift rounded-xl p-4 flex items-center justify-between gap-3"
          style={pyqOnly ? { boxShadow: `0 0 0 1px ${ACCENT}80` } : undefined}
        >
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              ...
              {pyqOnly && (
                <span style={{ color: availablePyqCount > 0 ? ACCENT : undefined }} className={availablePyqCount === 0 ? "text-rose-400" : ""}>
                  {availablePyqCount} available for this preset
                  {availablePyqCount < QUESTION_COUNT ? " — test will be shorter than usual" : ""}.
                </span>
              )}
            </p>
          </div>
          <div className="shrink-0 w-11 h-6 rounded-full transition-colors relative" style={{ backgroundColor: pyqOnly ? ACCENT : "var(--muted)" }}>
        ...
        <Card className="glass p-5 gap-0">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="secondary">{QUESTION_COUNT} MCQ · 1 mark each</Badge>
              <Badge variant="outline" style={{ color: ACCENT, borderColor: `${ACCENT}66` }}>
                Total: {QUESTION_COUNT} marks
              </Badge>
              <Badge variant="outline" className="text-amber-400 border-amber-500/30 bg-amber-500/10">
                <Clock className="size-3" /> {fmtMins(TEST_MINS)}
              </Badge>
            </div>
            <Button onClick={startTest} size="lg" style={{ backgroundColor: ACCENT, color: "#1a1306" }}>
              <Play /> Start Test
            </Button>
          </div>
        </Card>
            ...
            {questions.map((qq) => {
              ...
              return (
                <button
                  ...
                  style={
                    isCurrent
                      ? { backgroundColor: ACCENT, borderColor: ACCENT, color: "#1a1306" }
                      : ans
        ...
        <div style={{ "--sc": ACCENT } as CSSProperties}>
          <Card className="glass p-5 gap-0">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              <Badge variant="outline" className="text-xs" dir="auto">Ch {q.ch} · {chapterTitle(q.ch)}</Badge>
            ...
            <p dir="auto" lang="sa" className="font-semibold text-lg leading-snug mb-4 whitespace-pre-wrap">{q.q}</p>
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-border">
              ...
              <Button
                ...
                style={{ backgroundColor: ACCENT, color: "#1a1306" }}
              >
                Next <ArrowRight />
              </Button>
            </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight" style={accentStyle}>
            Sanskrit Mock Test Results 🎯
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            {preset === "all" ? "All Chapters" : PRESETS.find((p) => p.key === preset)?.label} · {QUESTION_COUNT} questions
          </p>
        </div>
            ...
            {wrongQs.map((q) => {
              ...
              return (
                ...
                >
                  ...
                  <p dir="auto" lang="sa" className="font-medium text-sm mb-3 whitespace-pre-wrap">{q.q}</p>
                  ...
                  <div className="rounded-md bg-muted/40 border border-border p-3">
                    ...
                    <div dir="auto"><Markdown text={q.exp} compact className="text-xs" /></div>
                  </div>
                </div>
              );
            })}
