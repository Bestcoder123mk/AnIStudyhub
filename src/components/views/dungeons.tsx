...
import { BossPanel, type BossEvent } from "@/components/shared/boss-panel";
import { getTrackChapters, getAllTrackMcqs, type TrackMCQ } from "@/lib/track-content";
...
export function DungeonsView() {
  ...
  const chapters = useMemo(() => getTrackChapters(track), [track]);
  const allMcqs = useMemo(() => getAllTrackMcqs(track), [track]);

  const [cleared, setCleared] = useState<Set<number>>(new Set());
  const [activeCh, setActiveCh] = useState<number | null>(null);
  const [questions, setQuestions] = useState<TrackMCQ[]>([]);
  ...
  // Same idea as the boss battle: HP is a live reflection of score against
  // the actual clear threshold, so "boss defeated" always lines up with
  // "you cleared this room" — not a separately tracked stat that can drift.
  const bossHp = Math.max(0, 100 - score * (100 / PASS_SCORE));
  ...
  const pickAnswer = (i: number) => {
    ...
    setSelected(i);
    bossHitCounter.current += 1;
    setBossEvent(
      correct
        ? { type: "hit", amount: Math.round(100 / PASS_SCORE), nonce: bossHitCounter.current }
        : { type: "miss", amount: 0, nonce: bossHitCounter.current }
    );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {mounted &&
          chapters.map((ch, i) => {
            ...
            // getTrackChapters() always returns `num` as a string — English's
            // raw data stores it as a bare number ("1"), while every other
            // track already bakes "Ch " into the string ("Ch 1"). Detect
            // which case this is by content, not by (now-always-string) type.
            const numLabel = /^\d+$/.test(ch.num) ? `Ch ${ch.num}` : ch.num || `Ch ${ch.id}`;
            return (
              <Card
                key={ch.id}
          ...
          {!finished && current ? (
            <div className="space-y-4">
              <BossPanel
                ...
                hp={bossHp}
                event={bossEvent}
                emojiOverride={activeCh !== null ? bossFor(chapters.findIndex((c) => c.id === activeCh)) : undefined}
                nameOverride={activeChapter ? `Guardian of ${activeChapter.title}` : undefined}
              />
