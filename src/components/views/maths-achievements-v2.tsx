...
// Maths accent (cyan) — matches SUBJECT_META.maths.accent
const ACCENT = SUBJECT_META.maths.accent; // single source of truth — was a duplicated hex literal

// Map achievement id -> { field, target, suffix, label } for progress hints.
// All fields exist on the SscState shape used by subjectStats["maths"].
const HINTS: Record<string, { field: string; target: number; suffix?: string; label?: string }> = {
  first_solve: { field: "totalCorrect", target: 1, suffix: " correct" },
  streak5: { field: "bestStreak", target: 5, label: "best", suffix: " streak" },
  solve25: { field: "totalCorrect", target: 25, suffix: " correct" },
  solve50: { field: "totalCorrect", target: 50, suffix: " correct" },
  all_chapters: { field: "chaptersOpened", target: 13, suffix: " / 13" },
  algebra_ace: { field: "mcqDone", target: 10, suffix: " MCQs" },
  geometry_guru: { field: "mcqDone", target: 10, suffix: " MCQs" },
  formula_master: { field: "mcqDone", target: 15, suffix: " MCQs" },
...
export function MathsAchievementsV2() {
  ...
  const xpFromBadges = MATHS_ACHIEVEMENTS.filter((a) => unlockedSet.has(a.id)).reduce(
    (sum, a) => sum + a.xp,
    0,
  );
  ...
  const summary = [
    ...
    { label: "Total", value: MATHS_ACHIEVEMENTS.length, accent: false },
    ...
    <div className="space-y-6">
      ...
      <div className="page-header animate-float-up">
        ...
        <p className="text-muted-foreground mt-2">
          12 subject-themed badges — earn bonus XP as you master Real Numbers, Algebra,
          Geometry, Trigonometry & more!
        </p>
      </div>
...
function MathsBadgeCardV2({
  ...
  return (
    <Card
      ...
      style={
        unlocked
          ? { boxShadow: `0 0 0 1px ${ACCENT}40`, borderColor: `${ACCENT}33` }
          : undefined
      }
    >
