...
interface SktCard {
  ch: number;
  chTitle: string; // Devanagari chapter title
  num: string;
...
// Build the deck: one card per chapter (front: chapter title, back: oneshot + keypts).
function buildSktCards(): SktCard[] {
  return SKT_CHAPTERS.map((ch, idx) => ({
    ch: ch.id,
    chTitle: ch.title,
    num: ch.num,
    idx,
    front: ch.title,
    back: [...ch.oneshot, ...ch.keypts].join("\n\n"),
  }));
}

const ALL_CARDS = buildSktCards();

export function SktFlashcardsView() {
  ...
  const rate = (quality: 2 | 3 | 5) => {
    if (!current) return;
    reviewFlash("sanskrit");
    rateSrCard(`sktflash-${current.ch}-${current.idx}`, quality);
    setTimeout(() => {
      setFlipped(false);
      setIdx((i) => (i + 1) % deckCards.length);
    }, 220);
    ...
    <div className="space-y-6">
      <header>
        <h1
          ...
          style={{ color: ACCENT }}
        >
          Sanskrit Flashcards 🕉️
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          {ALL_CARDS.length} cards · one per Shemushi chapter · oneshot + key grammar points · Click to flip · +5 XP per review
        </p>
      </header>
        ...
        <div style={wrapperStyle}>
          ...
          >
            <div className="flip-3d-inner h-full w-full">
              {/* Front — chapter title in Devanagari */}
              ...
              <div className="flip-3d-face flip-3d-back h-full w-full rounded-2xl border-2 border-subj bg-muted/30 p-5 sm:p-6 flex flex-col overflow-y-auto scroll-thin">
                ...
                <div dir="auto" lang="sa" className="text-sm sm:text-base leading-relaxed">
                  <Markdown text={current.back} compact />
                </div>
              </div>
            </div>
          </div>
